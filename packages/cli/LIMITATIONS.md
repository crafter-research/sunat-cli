...
- **SOL portal is geo-restricted.** From a non-Peruvian IP, `e-menu.sunat.gob.pe` and `e-consulta.sunat.gob.pe` reset the connection (DNS resolves, TCP 443 opens, reset on data). `www.sunat.gob.pe`, `e-factura`, `e-beta` and `api-seguridad` answer fine. Anything driven through agent-browser (`login`, `rhe`, `f616`, `padron ruc-online`, `tipo-cambio`) therefore needs a Peruvian route; the SOAP and REST surfaces do not.
